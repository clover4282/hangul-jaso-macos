...
- **폴더 감시**: 지정한 폴더(Downloads, Desktop, Documents 등)를 실시간 모니터링
- **자동 NFC 변환**: 감시 폴더에 NFD 파일이 생기면 자동으로 변환
- **NFD 태그 표시**: NFD 파일에 주황색 `NFD` 태그를 자동 부여
- **Finder 컨텍스트 메뉴**: Finder에서 파일 우클릭 시 "한글 파일명 NFC 변환" 메뉴 제공 (Finder Sync Extension)
- **Finder 빠른 동작**: Quick Action을 통한 NFC 변환
- **알림**: 변환 완료 시 macOS 알림 표시
...
├── Services/            # NFCService, FileMonitorService, WorkflowInstaller
├── ViewModels/          # HangulJasoViewModel
├── Views/               # SwiftUI 설정 화면
└── Resources/           # Info.plist, Assets

HangulJasoFinder/        # Finder Sync Extension
├── FinderSync.swift     # 컨텍스트 메뉴 "한글 파일명 NFC 변환"
...
Finder에서 파일/폴더를 우클릭하면 컨텍스트 메뉴에 "한글 파일명 NFC 변환" 항목이 표시됩니다. 이 기능은 Finder Sync Extension으로 구현되어, "빠른 동작" 서브메뉴가 아닌 직접 컨텍스트 메뉴에 나타납니다.
